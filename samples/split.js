...
draw.addEventListener(Datatang.DrawEvent.EventType.DRAW_END, function(
  drawEvent
) {
  ...
  var featureCollection = Datatang.splitPolygonByPolyline(
    splitedPolygon,
...
function getIntersectedGeometry(geometry) {
  ...
  var filters = features.filter(function(feature) {
    return (
      ...
      Datatang.intersects(geometry, feature.geometry)
    );
  });
